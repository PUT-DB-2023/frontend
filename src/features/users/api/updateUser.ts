...
export const updateUserNew = async (data: Student | Teacher | User, type: UserType) => {
    ...
    const response = await axios.patch(dest, data)
        ...
        .catch((e) => { toast.update(t, { render: `Nie udało się edytować użytkownika - ${displayError(e.response.data)}`, type: "error", theme: "colored", isLoading: false, closeButton: true, autoClose: 8000 }); return e })
    return response.data
}
