...
export const deleteCourse = async (id: string|undefined) => {
    ...
    .catch((e)=>{toast.update(t, {render: `Nie udało się usunąć przedmiotu - ${displayError(e.response.data)} - ${displayError(e.response.data)}`, type: "error", theme: "colored", isLoading: false, closeButton: true, autoClose: 12000}); return {status: 0, response: e}})
    return response
}