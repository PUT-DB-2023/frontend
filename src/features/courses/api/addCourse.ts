...
export const addCourse = async ({name, description}: {name: string, description: string}) => {
    ...
    .catch((e)=>{toast.update(t, {render: `Nie udało się dodać przedmiotu - ${displayError(e.response.data)} - ${displayError(e.response.data)}`, type: "error", theme: "colored", isLoading: false, closeButton: true, autoClose: 8000}); return e})
