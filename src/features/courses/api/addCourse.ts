...
export const addCourse = async ({name, description, major}: {name: string, description: string, major?: string}) => {
    ...
    .catch((e)=>{toast.update(t, {render: `Nie udało się dodać przedmiotu - ${displayError(e.response.data)}`, type: "error", theme: "colored", isLoading: false, closeButton: true, autoClose: 12000}); return e})
