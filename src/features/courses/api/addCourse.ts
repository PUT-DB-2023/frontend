...
export const addCourse = async ({name, description}: {name: string, description: string}) => {
    ...
    .then((e)=>{toast.update(t, {render: `Pomyślnie dodano przedmiot - ${e.data.name}`, type: "success", theme: "colored", isLoading: false, closeButton: true, autoClose: 5000}); return e})
