...
export const TeachersDropDown = ({ title, values, value, setValue }: any) => {
        ...
        <FieldBox title={title}>
            <Combobox value={value} onChange={(v) => setValue(v)} multiple>
                <div className="relative">
                    <div className={'relative w-full bg-zinc-50 rounded-md border border-zinc-400 h-9 hover:border-zinc-600 focus-within:outline-blue-800 focus-within:focus-visible:outline-blue-800 focus-visible:outline-blue-800'}>
                        <Combobox.Input
                            ...
                            displayValue={(options: any) => options ? options?.map((option: any) => option ? option?.first_name + ' ' + option?.last_name : '').join(', ') : ''}
                        />
                            ...
                            >
                                {({ selected }) => (         
                                        ...
                                        <div className={`${selected ? 'bg-blue-100' : 'hover:bg-zinc-100 [&>div]:hover:bg-blue-600'} flex gap-7 w-full`}>
                                            ...
                                            <span className={`${selected ? `font-normal text-blue-600` : `font-normal`} my-[6px]`}>{option.first_name + ' ' + option.last_name}</span>
                                        </div>                
                                        </> 
                                )}
                            </Combobox.Option>
